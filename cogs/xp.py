...
class xp(commands.Cog):
    ...
    async def level(self, ctx, member: discord.User):
        ...
        embed = discord.Embed(title=f"{member.name}'s level", color=0xfff)
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            embed = discord.Embed(title=f"{ctx.author.name}'s level", color=0xfff)
            embed.add_field(name="Level", value=user['level'])
            embed.add_field(name="XP", value=user['xp'])
            await ctx.send(embed=embed, delete_after=15)
    ...
    async def addrank(self, ctx, role: discord.Role, rank: int):
        ...
        if error:
            embed = discord.Embed(title=f"Couldn't create the reward!", color=0xfff)
            embed.add_field(name="Error", value=error)
            await ctx.send(embed=embed, delete_after=15)
        else:
            embed = discord.Embed(title=f"Reward created!", color=0xfff)
    ...
    async def removerank(self, ctx, level: int):
        ...
        if error:
            embed = discord.Embed(title=f"Could not remove reward!", color=0xfff)
            embed.add_field(name="Error", value=error)
            await ctx.send(embed=embed, delete_after=15)

        else:
            embed = discord.Embed(title=f"Removed reward!", color=0xfff)
    ...
    async def ranks(self, ctx):
        ...
        if not ranks:
            embed = discord.Embed(title=f"No reward!", color=0xfff)
            embed.add_field(name="There are no ranks!", value="To setup ranks type -addrank @rank level")
            await ctx.send(embed=embed, delete_after=15)
            return

        embed = discord.Embed(title=f"All ranks", color=0xfff)
        for r in ranks:
